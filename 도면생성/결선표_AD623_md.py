...
def 표(wires, parts, ics, 이름):
    ...
    n = 0
    for c0, nm, s1, s2, top, bot in [i for i in ics if len(i) > 4]:
        n += 1
        out.append(f'| {n} | **{nm}** | {c0}–{c0+3}열 · 홈을 걸친다 | 위 `{" · ".join(top)}` / '
                   f'아래 `{" · ".join(bot)}` | 왼쪽이 GND 쪽 |')
...
**AD623 입력 바이어스 전류는 17 nA typ · 25 nA max** (데이터시트 Rev. C p.3 DUAL SUPPLIES).
...
표기는 `(열, 행)` 이다. **두 소자 모두 가운데 홈을 걸치고, 왼쪽이 GND(또는 1번) 쪽이다.**
...
2. **전원부만 조립** (1번 보드의 R1 · R2 · 가상접지 버퍼) → 멀티미터로 V+ · V− 확인.
   **여기서 안 나오면 멈춘다**
3. **AD623 모듈에 핀 헤더를 납땜**하고 4–7열에 꽂는다. **왼쪽이 GND 쪽**
