...
# 전극을 채널마다 하나만 쓰므로 −쪽은 전극이 아니라 1 Ω 으로 GND 에 묶는다 (2026-08-29).
for (px,py),lab,rb,rv in ((pA,'—  GND 에 묶임','1 Ω','1 Ω'),(pB,'기록 전극','Rb1','1 MΩ')):
    ...
    end=950 if rv=='1 Ω' else 1070
    dot(210,py,SIG); res(210,py,210,end,rb,rv); gnd(210,end)
# 되먹임 Ra / Rb 와 이득저항 Rg
...
# 오디오 인터페이스로 — 10 µF 을 거쳐 잭 브레이크아웃의 TIP 으로
panel(1660,1240,760,270,'오디오 인터페이스로 (오실로스코프는 안 쓴다)')
w_([(2300,960),(2360,960),(2360,1350),(2060,1350)],'#2f2f2f',4)
res(2060,1350,1960,1350,'10 µF','전해 50 V')
a('<rect x="1720" y="1290" width="230" height="120" rx="10" fill="#4b4b4b" stroke="#333" stroke-width="2"/>')
a('<circle cx="1790" cy="1350" r="22" fill="#1c1f24" stroke="#8d939c" stroke-width="3"/>')
a('<circle cx="1790" cy="1350" r="9" fill="#0b0d10"/>')
T(1826,1344,'콤보 입력',16,'#eee','700')
T(1826,1366,'Line/Inst → Inst',14,'#a8bccd')
gnd(1845,1450,'잭의 SLEEVE')
w_([(1845,1410),(1845,1450)])
T(1690,1524,'잭 브레이크아웃이 보드 30–33열 J행에 직접 꽂힌다 · 컴퓨터는 배터리로 돌린다',17,'#111','700')
